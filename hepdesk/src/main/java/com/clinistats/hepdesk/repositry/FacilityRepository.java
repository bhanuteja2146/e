...
//	List<GetIdNameColor> findFacilitiesByNameSearch(Status recordState);
	...
	@Query("select new com.clinistats.hepdesk.response.GetIdName(f.id,f.name) from FacilityModel f where f.recordState = 0 and f.id = ?1 ")
	...
	@Query("select new com.clinistats.hepdesk.response.GetIdName(f.id,f.name) from FacilityModel f where f.id = ?1 ")
